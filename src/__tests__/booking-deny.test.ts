...
describe('POST /api/bookings/[id]/deny', () => {
      ...
      eq: vi.fn(() => ({
        select: vi.fn(() => ({
          single: vi.fn().mockResolvedValue({
            data: { id: 'booking-uuid-1', status: 'declined' },
            error: null,
          }),
        })),
      })),
  ...
  it('returns 200 and declines a pending booking', async () => {
    mockState.bookingStatus = 'pending';
    const { request, context } = makeRequest();
    const response = await POST(request, context);
  ...
  it('returns 422 for a confirmed booking and does not call update', async () => {
